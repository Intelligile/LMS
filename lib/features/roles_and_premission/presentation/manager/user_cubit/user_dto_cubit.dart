...
class UserDtoCubit extends Cubit<UserDtoState> {
  ...
  UpdateUserAuthoritiesUseCase updateUserAuthoritiesUseCase;
  UserDtoCubit(this.fetchUsersUseCase, this.updateUserAuthoritiesUseCase)
  ...
  Future<void> updateUserAuthorities(
    ...
    emit(UpdateUserAuthoritiesLoadingState());
    var result = await updateUserAuthoritiesUseCase.call(
        userId: userId, authoritiesId: authoritiesId);
    result.fold(
      (failure) {
        emit(UpdateUserAuthoritiesFailureState(errorMessage: failure.message));
      },
      (users) {
        emit(UpdateUserAuthoritiesSuccessState());
      },
    );
  }
}
